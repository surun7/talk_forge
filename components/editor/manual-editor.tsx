                  ...
                  <div className="pt-1"><span className="text-[10px] text-slate-400 uppercase tracking-wider block mb-2">Items</span>
                    ...
                    {cs.items.map((item: any, i2: number) => (
                      <CollapsibleItem key={item.id + "_" + i2 + "_" + cs.id} title={item.name || (item as any).text || `Item ${i2 + 1}`} open={!!openSections[item.id]} onToggle={() => toggle(item.id)} onMoveUp={() => moveCsItemInSection(cs.id, i2, -1)} onMoveDown={() => moveCsItemInSection(cs.id, i2, 1)} isFirst={i2 === 0} isLast={i2 === cs.items.length - 1} onDelete={() => onChange({ ...resume, customSections: resume.customSections.map(x => x.id === cs.id ? { ...x, items: x.items.filter((i: any) => i.id !== item.id) } : x) })}>
                        <Field label={t("custom.name")}><input className={inputCls} value={item.name || (item as any).text || ""} onChange={e => onChange({ ...resume, customSections: resume.customSections.map(x => x.id === cs.id ? { ...x, items: x.items.map((i: any) => i.id === item.id ? { ...i, name: e.target.value } : i) } : x) })} /></Field>
                        <Field label={t("custom.affiliation")}><input className={inputCls} value={item.affiliation || ""} onChange={e => onChange({ ...resume, customSections: resume.customSections.map(x => x.id === cs.id ? { ...x, items: x.items.map((i: any) => i.id === item.id ? { ...i, affiliation: e.target.value } : i) } : x) })} /></Field>
                        <Field label={t("custom.role")}><input className={inputCls} value={item.role || ""} onChange={e => onChange({ ...resume, customSections: resume.customSections.map(x => x.id === cs.id ? { ...x, items: x.items.map((i: any) => i.id === item.id ? { ...i, role: e.target.value } : i) } : x) })} /></Field>
                        <div className="flex gap-2">
                          <div className="flex-1"><Field label={t("custom.startDate")}><input className={inputCls} value={item.startDate || (item as any).time || ""} onChange={e => onChange({ ...resume, customSections: resume.customSections.map(x => x.id === cs.id ? { ...x, items: x.items.map((i: any) => i.id === item.id ? { ...i, startDate: e.target.value } : i) } : x) })} /></Field></div>
                          <div className="flex-1"><Field label={t("custom.endDate")}><input className={inputCls} value={item.endDate || ""} onChange={e => onChange({ ...resume, customSections: resume.customSections.map(x => x.id === cs.id ? { ...x, items: x.items.map((i: any) => i.id === item.id ? { ...i, endDate: e.target.value } : i) } : x) })} placeholder={t("certificates.present")} /></Field></div>
                        </div>
                        <FormattedField label={t("custom.description")} value={item.description || ""} onChange={v => onChange({ ...resume, customSections: resume.customSections.map(x => x.id === cs.id ? { ...x, items: x.items.map((i: any) => i.id === item.id ? { ...i, description: v } : i) } : x) })} />
                      </CollapsibleItem>
                    ))}
                    </div>
                    <button onClick={() => onChange({ ...resume, customSections: resume.customSections.map(x => x.id === cs.id ? { ...x, items: [...x.items, { id: "csi_" + nextId(), name: "", role: "", affiliation: "", startDate: "", endDate: "", description: "" }] } : x) })} className={btnCls}><Plus className="w-3 h-3" />{t("custom.addItem")}</button>
                  </div>
                </div>
              </AnimatedSection>
            </React.Fragment>
