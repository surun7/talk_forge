...
interface Props {
  ...
  sLabel: (k: string, v: string) => void; upBasic: (f: string, v: string) => void;
  onMoveUp?: () => void; onMoveDown?: () => void; isFirst?: boolean; isLast?: boolean;
}

export default function OverviewSection({ resume, onChange, openSections, toggle, visible, togVis, sectionTitle, sectionIconEl, sLabel, upBasic, onMoveUp, onMoveDown, isFirst, isLast }: Props) {
  return (<>
    <SectionHeader title={sectionTitle("overview", "OVERVIEW")} count={0} icon={sectionIconEl("overview", "file-text")} open={!!openSections.overview} onToggle={() => toggle("overview")}
      visible={visible("overview")} onToggleVisibility={() => togVis("overview")} sectionKey="overview" onTitleChange={sLabel}
