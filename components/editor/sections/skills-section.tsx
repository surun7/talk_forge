    ...
    <AnimatedSection open={!!openSections.skills}>
      <div className="px-2 pb-3 space-y-1">
        {resume.skills.map((cat, idx) => (
          <CollapsibleItem key={cat.id} title={cat.name || "New Category"} open={!!openSections[cat.id]} onToggle={() => toggle(cat.id)} onMoveUp={() => moveItem(idx, -1)} onMoveDown={() => moveItem(idx, 1)} isFirst={idx === 0} isLast={idx === resume.skills.length - 1} onDelete={() => onChange({ ...resume, skills: resume.skills.filter(x => x.id !== cat.id) })}>
            ...
            <div><span className="text-[10px] text-slate-400 uppercase tracking-wider block mb-2">Skills</span>
              ...
              <button onClick={() => { const newSkill = { id: nextId(), name: "" }; onChange({ ...resume, skills: resume.skills.map(x => x.id === cat.id ? { ...x, skills: [...x.skills, newSkill] } : x) }); }} className={btnCls}><Plus className="w-3 h-3" />Add Skill</button>
            </div>
          </CollapsibleItem>
        ))}
