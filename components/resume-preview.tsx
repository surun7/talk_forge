        ...
        {experience.length > 0 && !hidden(resume, "experience") && (
          <section className="mb-5">
            ...
            {experience.map((exp) => (
              <div
                key={exp.id}
                className="pb-2 mb-3 pl-3 border-l-2 a-border border-b-2 border-slate-300 dark:border-slate-600"
              >
        ...
        {certificates.length > 0 && !hidden(resume, "certificates") && (
          <section className="mb-5">
            ...
            {certificates.map((cert) => (
              <div
                key={cert.id}
                className="pb-1 mb-3 pl-3 border-l-2 a-border border-b-2 border-slate-300 dark:border-slate-600"
              >
                <div className="flex justify-between items-baseline">
                  <span className="font-bold text-[1em] text-slate-800">
                    {cert.name}
                ...
                {cs.items.length > 0 && (
                  <div className="space-y-3">
                    {cs.items.map((item: any, i2: number) => (
                      <div key={item.id + "_" + i2 + "_" + cs.id} className="pb-2 mb-3 pl-3 border-l-2 a-border border-b-2 border-slate-300 dark:border-slate-600">
                        <div className="flex justify-between items-baseline">
            ...
            {sec.items.length > 0 && (
              <div className="space-y-3">
                {sec.items.map((item: any, i2: number) => (
                  <div key={item.id + "_" + i2 + "_" + sec.id} className="pb-2 mb-3 pl-3 border-l-2 a-border border-b-2 border-slate-300 dark:border-slate-600">
                    <div className="flex justify-between items-baseline">
                      <span className="font-bold text-[1em] text-slate-800">{item.name || (item as any).text}</span>
                      <span className="text-[0.917em] text-slate-500 not-italic">
                        {item.startDate || (item as any).time}
